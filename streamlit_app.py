...
costo_variable_base = st.number_input("Monto de costo variable para el primer año:", value=40000.00)
...
for i in range(n_años):
    ...
    # Cálculo del costo variable con crecimiento
...
total_egresos = [cv + gf for cv, gf in zip(costos_variables, gastos_fijos)]
...
tabla = pd.DataFrame({
    ...
    "Ingresos": [0] + ingresos,
    "Costos Variables": [0] + costos_variables,
...
while abs(diferencia_van) > tolerancia:
    ...
    if ajuste_opcion == "Costo Variable":
        costos_variables_pesimistas = [nuevo_costo_variable_base * ((1 + crecimiento_costo_variable) ** i) for i in range(n_años)]
    else:
        costos_variables_pesimistas = [costo_variable_base * ((1 + crecimiento_costo_variable) ** i) for i in range(n_años)]

    if ajuste_opcion == "Gastos Fijos":
        gastos_fijos_pesimistas = [nuevo_gasto_fijo_base * ((1 + crecimiento_gastos_fijos) ** i) for i in range(n_años)]
    else:
        gastos_fijos_pesimistas = [gasto_fijo_base * ((1 + crecimiento_gastos_fijos) ** i) for i in range(n_años)]

    costos_de_ventas_pesimistas = [ingreso * cv for ingreso, cv in zip(ingresos_pesimistas, costos_variables_pesimistas)]
    total_egresos_pesimistas = [cv + gf for cv, gf in zip(costos_de_ventas_pesimistas, gastos_fijos_pesimistas)]
    utilidad_neta_pesimista = [ingreso - egreso for ingreso, egreso in zip(ingresos_pesimistas, total_egresos_pesimistas)]
...
# Mostrar los resultados pesimistas
st.write(f"**Nuevo VAN ajustado:** ${van_pesimista:,.2f}")
if ajuste_opcion == "Ingresos":
    st.write(f"**Nuevos Ingresos Base para alcanzar VAN de {van_objetivo}:** ${nuevo_ingreso_base:,.2f}")
elif ajuste_opcion == "Costo Variable":
    st.write(f"**Nuevo Costo Variable Base para alcanzar VAN de {van_objetivo}:** ${nuevo_costo_variable_base:,.2f}")
elif ajuste_opcion == "Gastos Fijos":
    st.write(f"**Nuevos Gastos Fijos Base para alcanzar VAN de {van_objetivo}:** ${nuevo_gasto_fijo_base:,.2f}")
